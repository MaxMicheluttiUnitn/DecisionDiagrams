	...
	do
		...
		do
			...
			if [ -f "$outputprobs"/"$jsonfilename" ]; then
				...
				if [ -f "$tmpfile" ]; then
					timeout 3600s python main.py -i "$item" --load_details "$jsontmpfile" --load_lemmas "$tmpfile"  --tsdd --count_nodes --count_models -d "$outputprobs"/"$jsonfilename" --tvtree balanced
					if [ $? -eq 0 ]; then
