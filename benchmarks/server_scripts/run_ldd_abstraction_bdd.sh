...
outputfolder="./benchmarks/ldd/output/abstraction_bdd/"

mkdir ./benchmarks/ldd/output/
mkdir ./benchmarks/ldd/output/abstraction_bdd/
...
timeout 3600s python main.py -i "$lddbenchfolder""$smtfilename" --pure_abstraction --bdd --count_models --count_nodes -d "$outputfolder""$jsonfilename"
        if [ $? -eq 0 ]; then
