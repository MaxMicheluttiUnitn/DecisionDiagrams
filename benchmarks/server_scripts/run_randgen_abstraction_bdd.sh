...
mkdir ./benchmarks/randgen/output_abstraction
    ...
    do
        ...
        do
            ...
            if [ -f "$outputprobs"/"jsonfilename" ]; then
                ...
                timeout 3600s python main.py -i "$item" --bdd --count_nodes --count_models --pure_abstraction -d "$outputprobs"/"$jsonfilename"
                if [ $? -eq 0 ]; then
