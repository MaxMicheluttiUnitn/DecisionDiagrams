...
do
	...
	if [ -f "$output_folder_xor""$jsonfilename" ]; then
		...
		echo "Performing task on $smtfilename"
		if [ -f "$tmpsmtfilename" ]; then
timeout 3600s python main.py -i "$wmi_data_folder_xor""$smtfilename" --load_lemmas "$tmpsmtfilename" --sdd --count_models --count_nodes -d "$output_folder_xor""$jsonfilename"
			if [ $? -eq 0 ]; then
				echo "Task completed on $smtfilename"
			else
				echo "DD Timeout on $smtfilename"
