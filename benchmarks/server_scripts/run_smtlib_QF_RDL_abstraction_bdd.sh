...
mkdir ./benchmarks/smtlib/output_abstraction_bdd/non-incremental/QF_RDL
...
do
    ...
    do
        ...
        if [ -f "$outputfolder"/"$jsonfilename" ]; then
            ...
            timeout 3600s python main.py -i "$item" --bdd --count_nodes --count_models --pure_abstraction -d "$outputfolder"/"$jsonfilename"
            if [ $? -eq 0 ]; then
