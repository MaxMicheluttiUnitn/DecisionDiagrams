...
mkdir ./benchmarks/randgen/output_sdd_total

for gen in ./benchmarks/randgen/data/*
do
	outputgen="${gen/data/output_sdd_total}"
	tmpgen="${gen/data/tmp}"
	mkdir $outputgen
	mkdir $tmpgen
	for probs in $gen/*
	do
		outputprobs="${probs/data/output_sdd_total}"
		...
		do
			...
			if [ -f "$outputprobs"/"$jsonfilename" ]; then
				...
				if [ -f "$tmpfile" ]; then
					timeout 3600s python main.py -i "$item" --load_details "$outputprobs"/"$jsonfilename" --load_lemmas "$tmpfile"  --tsdd --count_nodes --count_models -d "$outputprobs"/"$jsonfilename" --tvtree balanced
					if [ $? -eq 0 ]; then
					...
					#timeout 3600s python main.py -i "$item" --save_lemmas "$tmpfile" --solver total -d "$outputprobs"/"$jsonfilename" --count_models
					#if [ $? -eq 0 ]; then
					#	timeout 3600s python main.py -i "$item" --load_details "$outputprobs"/"$jsonfilename" --load_lemmas "$tmpfile"  --tsdd --count_nodes --count_models -d "$outputprobs"/"$jsonfilename" --tvtree balanced
					#	if [ $? -eq 0 ]; then
					#		echo "Task completed on $smtfilename"
