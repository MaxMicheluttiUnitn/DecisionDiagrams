...
mkdir ./benchmarks/randgen/output
mkdir ./benchmarks/randgen/tmp


for gen in ./benchmarks/randgen/data/*
do
	outputgen="${gen/data/output}"
	tmpgen="${gen/data/tmp}"
	mkdir $outputgen
	mkdir $tmpgen
	for probs in $gen/*
	do
		outputprobs="${probs/data/output}"
		...
		do
			...
			if [ -f "$outputprobs"/"$jsonfilename" ]; then
				...
				if [ -f "$tmpfile" ]; then
					timeout 3600s python main.py -i "$item" --load_lemmas "$tmpfile"  --bdd --count_nodes --count_models -d "$outputprobs"/"$jsonfilename"
					if [ $? -eq 0 ]; then
