...
timeout 3600s python main.py -i "$lddbenchfolder""$smtfilename" --abstraction_sdd --count_models --count_nodes -d "$outputfolder""$jsonfilename" --abstraction_vtree balanced
        if [ $? -eq 0 ]; then
